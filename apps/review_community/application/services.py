...
from .external.tmdb import TMDBClient  # ⬅️ TMDB에서 영화 정보를 가져오는 모듈이라고 가정
...
class CommentAppService:
    def __init__(self, comment_thread_repository):
    ...
    def add_comment_to_movie(self, author_user_instance, request_dto):
        # 🔹 1. 해당 영화가 DB에 있는지 확인하고 없으면 TMDB에서 가져와 저장
        try:
            movie = MovieModel.objects.get(id=request_dto.movie_id)
        except MovieModel.DoesNotExist:
            tmdb_data = TMDBClient().fetch_movie_by_id(request_dto.movie_id)
            if not tmdb_data:
                raise ValueError("TMDB에서 해당 영화 정보를 찾을 수 없습니다.")

            movie = MovieModel.objects.create(
                id=request_dto.movie_id,
                korean_title=tmdb_data['title'],
                original_title=tmdb_data['original_title'],
                plot=tmdb_data['overview'],
                release_date=tmdb_data['release_date'],
                poster_image_url=tmdb_data['poster_path'],
            )
